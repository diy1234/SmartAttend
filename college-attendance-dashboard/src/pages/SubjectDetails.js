...
export default function SubjectDetails() {
  ...
  return (
    <div className="p-6 min-h-screen bg-gray-50">
      ...
      {/* SUBJECT MANAGEMENT PANEL (merged) */}
      <div className="bg-white rounded-xl shadow p-4 mb-6">
        <div className="flex items-center justify-between mb-3">
          <div className="font-medium">Manage Subjects</div>
          <div className="text-sm text-gray-500">Create, assign, or remove subjects</div>
        </div>

        <form onSubmit={handleCreateSubject} className="flex gap-3 items-center mb-4">
          <input
          ...
          />
          <select
            value={selectedDeptForNewSubject}
            onChange={(e) => setSelectedDeptForNewSubject(e.target.value)}
          ...
          <button type="submit" disabled={actionLoading} className="px-3 py-2 bg-blue-800 text-white rounded">
            {actionLoading ? "Working..." : "Create Subject"}
          </button>
        </form>
            ...
            <tbody>
              {departments.map((d) => (
                <tr key={d.id} className="border-t align-top">
                  <td className="p-2 font-medium w-48">{d.name}</td>
                  <td className="p-2">
                    <div className="flex flex-wrap gap-3">
                      {Array.isArray(d.subjects) && d.subjects.length > 0 ? (
                        d.subjects.map((s) => (
                          <div key={s.id || s.name} className="border rounded px-3 py-2 bg-gray-50">
                            <div className="font-medium">{s.name}</div>
                            <div className="text-xs mt-1 flex gap-2">
                              <button
                                onClick={() => {
                                  /* keep existing "View" behavior (no navigation change) */
                                }}
